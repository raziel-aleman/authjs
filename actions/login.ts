...
import { sendVerificationEmail, sentTwoFactorTokenEmail } from "@/lib/email";
import { getTwoFactortTokenByEmail } from "@/data/two-factor-token";
...
export const login = async (values: z.infer<typeof LoginSchema>, callbackUrl?: string) => {
    ...
    if (existingUser.isTwoFactorEnabled && existingUser.email) {
        if (code) {
            const twoFactorToken = await getTwoFactortTokenByEmail(
                existingUser.email
            );
